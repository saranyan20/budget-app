                    ...
                    <CRow>
                        ...
                        >
                            <CInput
                                ...
                                :lazy="false"
                                :value.sync="
                                    $v.incomeCategory.category_name.$model
                                "
                            ...
                            <CInput
                                ...
                                label="Sequence"
                                :lazy="false"
                                :value.sync="$v.incomeCategory.sequence.$model"
...
export default {
    data() {
        return {
            ...
            update: false,
            addSequence: '',
            incomeCategory: this.getEmptyForm(),
            incomeCategoryList: [],
            categoryNameFeedback: '',
            sequenceFeedback: '',
    ...
    computed: {
        formString() {
            return JSON.stringify(this.incomeCategory, null, 2);
        },
        isValid() {
            return !this.$v.incomeCategory.$invalid;
        },
        isDirty() {
            return this.$v.incomeCategory.$anyDirty;
        }
    },
            ...
            category_name: {
                ...
                isValidCategory(val) {
                    for (const [
                        index,
                        el
                    ] of this.incomeCategoryList.entries()) {
                        if (
    ...
    methods: {
        ...
        checkIfValid(fieldName) {
            const field = this.$v.incomeCategory[fieldName];
            if (!field.$dirty) {
                return null;
            }
        ...
        reset() {
            this.incomeCategory = this.getEmptyForm();
            this.$v.$reset();
        },
        ...
        addCategory() {
            this.validate();
            if (this.incomeCategory && this.isValid) {
                this.$apollo
                    .mutate({
                        mutation: queries.INSERT_INCOME_CATEGORY,
                        variables: this.incomeCategory
                    })
                    .then(res => {
                        const result =
                            res?.data?.insert_budget_app_income_category?.returning;
                        if (result[0].id > 0) {
                            this.addModal = false;
                            this.$apollo.queries.categoryData.refetch();
                            this.incomeCategoryList.push(this.incomeCategory);
                            this.reset();
                        }
                    })
                    .catch(err => {
        ...
        updateCategory() {
            ...
            if (this.isValid) {
                this.$apollo
                    .mutate({
                        mutation: queries.UPDATE_INCOME_CATEGORY,
                        variables: this.incomeCategory
                    })
                    .then(res => {
                        const result =
                            res?.data?.update_budget_app_income_category?.returning;
                        if (result[0].id > 0) {
                            this.addModal = false;
                            this.$apollo.queries.categoryData.refetch();
                        }
        ...
        deleteCategory() {
            this.$apollo
                .mutate({
                    mutation: queries.DELETE_INCOME_CATEGORY,
                    variables: {
                ...
                .then(res => {
                    const result =
                        res?.data?.delete_budget_app_income_category?.returning;
                    if (result[0].id > 0) {
                        this.$apollo.queries.categoryData.refetch();
    ...
    apollo: {
        categoryData: {
            query: queries.GET_INCOME_CATEGORY,
            update: data => data,
            result({ data }) {
                this.update = false;
                this.incomeCategoryList = data?.budget_app_income_category;
            }
        }
    }
};
